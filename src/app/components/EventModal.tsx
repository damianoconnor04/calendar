...
interface EventModalProps { clickedId: string | null, eventNames: { [key: string]: { name: string, color: Color } }, setEventNames: React.Dispatch<React.SetStateAction<{ [key: string]: { name: string, color: Color } }>>, eventModal: boolean; setEventModal: React.Dispatch<React.SetStateAction<boolean>>, removeEvent: (id: string) => void }
const EventModal: React.FC<EventModalProps> = ({ clickedId, eventNames, setEventNames, eventModal, setEventModal, removeEvent }) => {
  ...
  const setColor = (color: Color) => {
    setEventColor(color)
    if (clickedId) setEventNames(prevNames => ({ ...prevNames, [clickedId]: { name: eventNames[clickedId]?.name || 'New event', color: color } }))
  }
  ...
  return (
    <>
      ...
      <div className={`animate-fade animate-duration-200 animate-delay-50 w-full max-w-sm max-h-96 h-full bg-white rounded-xl flex flex-col shadow z-[9999]`} style={{ position: 'absolute', top: `${top}px`, left: `${left}px` }}>
        <header className='bg-[#f1f3f4] border-b border-b-[#f1f3f4] flex justify-end rounded-t-xl p-1'>
        ...
        <div className='flex flex-1 flex-col p-2'>
          <label htmlFor={`${hour}`} className='self-end max-w-[calc(90%_-_0.75rem)]'>
            <input placeholder={eventNames[clickedId].name.toString()} onChange={handleEventNameChange} className='pr-16 text-xl w-full border-b-2 border-b-gray-400/70 focus-visible:outline-none focus-visible:border-b-sky-400' />
          </label>
        ...
        <div className='flex items-center gap-1 ml-[calc(10%_+_1.25rem)]'>
